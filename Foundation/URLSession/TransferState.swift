...
extension _HTTPURLProtocol._TransferState {
    ...
    func byAppending(headerLine data: Data) throws -> _NativeProtocol._TransferState {
        ...
        guard let h = parsedResponseHeader.byAppending(headerLine: data, isHeaderComplete: isCompleteHeader) else {
        ...
        if case .complete(let lines) = h {
            ...
            return _NativeProtocol._TransferState(url: url, parsedResponseHeader: _HTTPURLProtocol._ParsedResponseHeader(), response: response, requestBodySource: requestBodySource, bodyDataDrain: bodyDataDrain)
        } else {
