...
extension _FTPURLProtocol._ParsedResponseHeader {
    ...
    func byAppending(headerLine data: Data) -> _FTPURLProtocol._ParsedResponseHeader? {
    ...
    private func byAppending(headerLine line: String) -> _FTPURLProtocol._ParsedResponseHeader {
        
        if line.starts(with: "150") {
            switch self {
            case .partial(let header): return .complete(header)
            case .complete: return .partial(_FTPURLProtocol._ResponseHeaderLines())
            }
        } else {
    ...
    private var partialResponseHeader: _FTPURLProtocol._ResponseHeaderLines {
        switch self {
        case .partial(let header): return header
        case .complete: return _FTPURLProtocol._ResponseHeaderLines()
        }
    }
}
private extension _FTPURLProtocol._ResponseHeaderLines {
    /// Returns a copy of the lines with the new line appended to it.
    func byAppending(headerLine line: String) -> _FTPURLProtocol._ResponseHeaderLines {
        var l = self.lines
        l.append(line)
        return _FTPURLProtocol._ResponseHeaderLines(headerLines: l)
    }
}
internal extension _FTPURLProtocol._ResponseHeaderLines {
    
    func createURLResponse(for URL: URL) -> URLResponse? {
        
        return URLResponse(url: URL, mimeType: nil, expectedContentLength: Int(32),textEncodingName: nil)
    }
...
extension _FTPURLProtocol {
    ...
    struct _FTPMessage {
        let startLine: _FTPURLProtocol._FTPMessage._StartLine
        let headers: [_FTPURLProtocol._FTPMessage._Header]
    }
}

extension _FTPURLProtocol._FTPMessage {
...
extension _FTPURLProtocol._FTPMessage._Version {
...
private extension _FTPURLProtocol._FTPMessage._StartLine {
    init?(line: String) {
...
private func createHeaders(from lines: ArraySlice<String>) -> [_FTPURLProtocol._FTPMessage._Header]? {
    
    var headerLines = Array(lines)
    var headers: [_FTPURLProtocol._FTPMessage._Header] = []
    while !headerLines.isEmpty {
        guard let (header, remaining) = _FTPURLProtocol._FTPMessage._Header.createOne(from: headerLines) else { return nil }
        headers.append(header)
        headerLines = remaining
    }
    return headers
}
private extension _FTPURLProtocol._FTPMessage._Header {
    ...
    static func createOne(from lines: [String]) -> (_FTPURLProtocol._FTPMessage._Header, [String])? {
        ...
        do {
            ...
            return (_FTPURLProtocol._FTPMessage._Header(name: name, value: value ?? ""), Array(t))
        }
    }
}
