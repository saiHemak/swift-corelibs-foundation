    ...
    fileprivate func box_<T : Encodable>(_ value: T) throws -> NSObject? {
        if T.self == Date.self  || T.self == NSDate.self {
          ...
          // return (value as! NSDecimalNumber)
            let deciVal = value as! Decimal
            return try self.box(deciVal.doubleValue)
        }
...
fileprivate class _JSONReferencingEncoder : _JSONEncoder {
    ...
    fileprivate init(referencing encoder: _JSONEncoder, key: CodingKey, ConvertedKey: CodingKey, wrapping dictionary: NSMutableDictionary) {
        self.encoder = encoder
...
extension _JSONDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        if let number = _SwiftValue.store(value) as? NSNumber {
            // TODO: Add a flag to coerce non-boolean numbers into Bools?
            if number ==  NSNumber(booleanLiteral: (kCFBooleanTrue != nil)) {
                return true
            } else if (number == NSNumber(booleanLiteral: (kCFBooleanFalse != nil))) {
                return false
